...
| 因子值按 `dataset_snapshot_id` 隔离存储；因子评估表携带快照血缘，内置评估器输出并持久化 Newey–West HAC、确定性 moving-block bootstrap，以及显式日期 holdout 的 IC/Rank IC 证据，发现任务输出 Benjamini–Hochberg FDR 证据；外部风险账本已支持 PIT 单暴露与联合 Fama–MacBeth/HAC 诊断 | rolling/多 horizon 的独立验证、风险模型官方源与“联合暴露/bootstrap/因子 OOS 何时成为晋级硬门”的真实样本校准仍待补齐 |
...
| Math Harness 有数值、反例、量纲、敏感性和符号检查；Quant Stats 已具备 HAC、确定性 moving-block bootstrap、显式 holdout、FDR、信号 VIF/相关性与外部联合暴露诊断 | 尚未覆盖官方风险模型源、rolling/多 horizon 独立验证和晋级阈值的生产样本校准 |
...
| recommendation outcome 可生成保守的 confirmed / invalidated / inconclusive reflection，并明确非因果归因限制；显式绑定 `thesisId` 的主周期结果还会带着市场数据指纹写回其 Forecast Book | 仍需将真实成交、执行偏差与容量衰减纳入策略归因；组件升级只能使用冻结 cohort 的 offline + paper 对照，shadow 仅供运行安全与信号观测审计，不能由单次结果自动推广 |
...
- 已完成：live 的日历 release 门。`exchange-calendar-release` 是无 I/O、可单独加载/卸载的严格适配层：连接器可将官方交易所或持牌供应商的 `source/version/retrievedAt/venue/timezone/effective range`、逐日状态和可选早收盘/分段窗口冻结成 release。每个 real-money runtime 必须绑定该 release（可用 `calendarVenue` 区分资产市场和实际交易所）；它是 live 运行时唯一的会话权威，缺失、来源不合格、venue 不符、有效期外、当天未显式列出、闭市或窗口外均停止信号和下单，并限频写审计日志，既不会退回“周一至周五”的默认推断，也不会让通用常规时段覆盖有效的早收盘/分段窗口。日历下载连接器仍可独立接入，卸载它只会阻止新 live 运行时准入，不能放宽已实现的订单和会话硬门。
- 已完成：live 显式项目风控门。零条项目级 `pre_trade` 规则时，paper/simulation 仍可写出可观测的 `paper_default_allow`，但 real-money intent 会以 `live_pre_trade_rules_missing` 进入拒绝任务；它不能继承为演示准备的默认放行。运行时的版本/日历/HITL/名义金额/日损失 envelope 与项目级规则分别在 intent 创建和 broker dispatch 前复核，任一缺失均 fail-closed。
- 已完成：live 账户风险快照门。`live-account-risk` 为无状态、可独立装卸的账户风险策略；v2 runtime envelope 必须声明 USD 最低可用现金、gross/symbol 名义金额和最大持仓数。broker dispatch 前它实时读取余额与持仓：余额必须给出可用 USD，任一非零持仓必须给出 broker mark 或市值，缺失时拒绝而不拿成本均价或缓存替代；再以投影后的净标的、gross 与持仓数执行限额检查。账户桥只负责取得 snapshot，风险逻辑不依赖具体券商；卸载/失效的桥会使 live 订单因 snapshot 不可得而保持 fail-closed。
- 已完成：执行质量进入评估证据。TCA 报告可按 `strategyRuntimeId` 过滤，避免混淆不同策略的成交；paper 评估将同一 runtime 的成交数、成交率、实现短缺、延迟、拒单率和费用持久化到 `strategy_eval_run.metricsJson.executionQuality`，同时作为 component evidence 的审计字段。现阶段这些指标是观察证据，不以任意拍定阈值悄然改变 pass/promotion；待积累同券商/资产/频率的真实 cohort 后，再显式版本化并校准执行质量晋级门。
- 已完成：成交成本按实际执行上下文归因。PnL materializer 从 runtime 的 `broker_account.provider` 和 `order_intent.instrument_id → instrument.asset_class` 读取费用维度，因而可以分别命中股票、期权、期货与虚拟币以及不同券商的版本化费率；缺少 instrument 分类时不会把衍生品猜作股票，只以 `unknown` 留下缺口（加密市场除外），避免虚构成本证据。
...
- 已完成：因子统计与准入最小闭环。`FactorResearchContract` 以轻量、可卸载的 `definition_json.researchContract` 保存经济机制、PIT 可得性、与可执行表达式一致的公式、预处理、适用域、失效条件及独立验证计划。内置因子评估按日横截面输出 Newey–West HAC 标准误、t 值、p 值与正 IC 比例，并以固定短哈希种子运行 500 次 moving-block bootstrap，持久化 IC/Rank IC 的 95% 区间与正向概率；至少 60 个日截面且 IC 或 Rank IC 显著才可通过。报告作为 `factor_evaluation.statistical_report_json` 持久化，因子发现对完整候选族做 Benjamini–Hochberg FDR 校正，并把方法、候选数、发现数及逐候选 adjusted p-value 一同返回。当前 bootstrap 是独立审计证据，尚未在缺少真实样本阈值校准时悄然替代 HAC 晋级门。`strategy.compose` 的显式选因子和自动补全，以及 `factor.promote_backtest` 的快捷路径，都会拒绝缺合同、未通过 HAC、未绑定冻结快照或（回测时）快照不匹配的因子；未经校正的 IC 排名不可伪装成验证结论。
- 已完成：显式因子独立验证。`factor.autoEvaluate` 可接收 `validation_start_date`，在同一数据集快照上把评估拆为训练与 OOS；训练段会将最后 `horizon_days` 个自然日从标签侧排除，避免持有期收益跨越验证边界。in-sample 与 out-of-sample 的 IC/Rank IC/IR/样本量/统计报告、切分日期、purge 天数和 fail-closed 原因嵌入**同一** `factor_evaluation.statistical_report_json.independentValidation`，不会创建第二行含义不明的评估记录。缺少足够 OOS 样本只标 `research_only`；该报告尚未自动放宽或收紧既有 HAC 晋级门，等待真实样本阈值校准。
...
- 已完成：因子外部风险回归与 live 闭环。`factor.risk_exposure.regress` 对同一快照的单因子值，只匹配决策时间前已发布的外部暴露：保留逐维横截面 OLS beta/R²/覆盖缺口，同时提供联合 Fama–MacBeth 截面回归与系数均值的 Newey–West HAC 统计。共同维度缺覆盖、截面过小或秩亏会明确出现在 `joint` 诊断中，绝不被描述为中性；该新联合视图暂为研究审阅证据，尚未偷偷改变既有 promotion 语义。因子组合的 backtest evaluation 会持久化 v1 `factorRiskExposure`；若它声明 `required=true`，Champion–Challenger 与 live promotion 都必须读取 `status=passed`，否则不得晋级。非因子/历史记录不被追溯性阻断。
...
- 已完成：策略 Champion–Challenger 的同 cohort 比较门。backtest 与 walk-forward 会写入由冻结 snapshot、时框、标的池、universe、benchmark 和成本模型生成的 `strategy_cohort_*`；paper runtime 只能在同一策略已具备该 backtest + walk-forward 证据时引用该 ID，shadow 可带该 ID 保留观测关联但不能成为晋级证据。`strategy.champion_challenger.compare` 仅在 backtest、walk-forward、paper 三者均共享同 cohort 时计算分数，缺失/歧义/不匹配时明确返回不可比较，绝不自动切换 live runtime。
- 已完成：策略晋级的组合增量门。Walk-Forward 现持久化由冻结 OOS equity curve 导出的 `oosReturnSeries`；Champion–Challenger 只配对完全相同时间戳的收益，要求至少 60 个观测，并输出相关性、等权组合与 champion 的期间 Sharpe/波动增量。相关性过高、组合风险调整收益没有改善或旧记录缺该 OOS 序列时，候选会写入墓地为 rejected/incomplete，不能成为 `candidate_for_manual_promotion`。该固定 50/50 检查不搜索权重，避免晋级比较本身成为新的调参面。
...
- 已完成：通用组件的冻结 benchmark 对比。`component_eval_run` 现在把 `comparison_cohort_id` 作为结构化证据；Agent、Prompt、Tool、Model、Skill、数据源与 Harness 的控制/挑战版本只能在显式指定的同一 cohort 内比较。候选版本必须达到样本下限、所有记录通过，并同时拥有 offline 与 paper 证据；shadow 仅作为零下单的运行安全/信号观测审计。缺 cohort 的历史行仍可审计但不会参与推广。所有晋级仍只生成 `candidate_for_manual_promotion`，live 流量一律留在 control。
...
- 已完成：执行质量的校准合同。`execution-quality-contract` 是无 I/O、可独立加载的 TCA 评估器；runtime 可声明其券商、资产类别、频率、最小订单样本和滑点/成交率/延迟/拒单率阈值的历史 `calibrationId`。Paper 评估只在精确 scope 且样本充足时给出 `passed/failed`，否则明确标为 `not_configured`、`scope_mismatch` 或 `insufficient_sample`。该结果保留在策略评估证据中，但当前**不会**改变 paper pass、自动调参、策略晋级或订单派发，避免未经审批的阈值偷偷变成交易政策。
...
- 已完成：成交结果的可检索范围修复。PnL materializer 写 `pnl_episode` 经验时，会沿 `strategy_runtime → strategy_script → workflow/session` 解析真实 project；仅在两条血缘都不存在的旧记录上保留 strategy-private scope。这样研究 Agent 可以检索本项目的实际成交/费用/换手结果，且不会把旧策略结果泄露到其他项目；该经验仍只是观察事实，不能自行改变参数或下单。
...
- 已完成：`limited_live` 的 runtime deployment envelope。真钱 runtime 还必须在 `params.liveGuardrails` 写入版本化、有限的允许标的、单笔/单日名义金额、单日订单数、单日亏损上限与强制人工确认；创建与启动会检查 live broker account，`order_intent` 创建及真实 broker 派发前都重读同一 envelope。没有 runtime、账户/标的错配、超过额度/亏损上限或绕过确认的请求均 fail-closed，不会作为可重试的连接器错误重新派发。
- 已完成：部署语义去歧义。`paper` 是本地模拟成交，`sim` 是券商模拟账户，`shadow` 是只记录信号/目标仓位、**绝不**创建 order intent / execution task / broker 请求 / 仓位变动的运行模式，`limited_live` 是受 runtime guardrail 约束的真钱部署；其中 `strategy_eval_run.eval_kind='live'` 的旧兼容桶不再被泛化理解为“已人工批准”。只有 `scenarioKey='live_approval'`、`manual_limited_live_deployment_v2` 和完整 `strategy-live-admission-v1` 的记录才是人工部署授权：首个策略显式标为 `manual_champion_bootstrap`，已有 champion 时则必须携带当前冻结 cohort 的 `challenger_replacement` 与通过的 OOS 低相关/增量组合证据。旧的 label-only `v1` 批准需要重新人工签署；真实 live 绩效观察不能被覆盖为批准，也不能替代批准。shadow 可在交易模块暂停时继续用于研究观察，不能借其绕过任何实盘闸门。
...
### 真实环境 UAT（不得由本地 mock 替代）

以下检查是本轮代码门禁之外、提高真实资本权限前必须保留的运行证据；每项均应保存实际的 snapshot、runtime、broker account 与时间戳，而不是以截图或模型文本代替。

1. **数据与日历**：为至少一个美股、期权/期货与虚拟币交易所分别冻结可交易日、早收盘/分段 session、市场快照与成本版本；在闭市、早收盘边界和缺失 release 时验证 live runtime 与 broker dispatch 均 fail-closed。
2. **Shadow → Paper**：让同一策略连续运行并比对真实行情下的 signal、理论成交和 paper fill；核验 shadow 始终没有 `order_intent` / `execution_task` / 券商请求，且 Paper TCA 只归属于对应 runtime。
3. **晋级与人工签署**：在一个冻结 cohort 中构造低相关、增量为正的 challenger，确认其产生 `challenger_replacement` 签署记录；再构造高相关和缺少 OOS 的 challenger，确认两者无法创建该记录。历史 `v1` label-only approval 必须在 UAT 数据库中不可启动真钱 runtime。
4. **券商与有限真钱**：在 sandbox 后、经人工确认的最小额度真实账户上验证白名单、单笔/单日名义金额、频率、日损失、账户可用现金/标记、项目 pre-trade rule 与暂停开关的拒绝原因；人工取消与 broker 已确认订单的撤单/对账必须单独留痕。
5. **结果与反思**：注入真实（或经券商可核验的）fill/mark，重跑 PnL worker，核对 `strategy_pnl_snapshot`、项目范围 `pnl_episode`、技能归因、分析师准确率和 Forecast Book 的来源链接；确认重复 worker 不累加伪样本，也不会自动修改策略参数、晋级状态或下单权限。
6. **恢复演练**：在 pending review、shadow、paper 与 limited-live 运行中重启服务；验证持久化暂停状态、审批记录、限额和运行时证据仍被重新读取，任何 snapshot/日历/账户风险数据不可得时保持 fail-closed。
