-- 0109_broker_qmt_provider — QMT / xtquant native A-share trading provider.

PRAGMA foreign_keys=OFF;
--> statement-breakpoint
CREATE TABLE broker_account_new (
  id TEXT PRIMARY KEY,
  provider TEXT NOT NULL CHECK(provider IN ('futu','ib','ccxt','alpaca','supermind','eastmoney_emt','qmt')),
...
--> statement-breakpoint
INSERT INTO broker_account_new SELECT id, provider, account_ref, mode, base_url, enabled, health_status, health_message,
  last_health_at, provider_config_json, is_default, created_at, updated_at FROM broker_account;
--> statement-breakpoint
...
CREATE TABLE broker_order_event_new (
  ...
  provider TEXT NOT NULL CHECK(provider IN ('futu','ib','ccxt','alpaca','supermind','eastmoney_emt','qmt')),
  event_type TEXT NOT NULL CHECK(event_type IN ('submit','ack','partial_fill','fill','cancel','modify','reject','health_check')),
...
--> statement-breakpoint
INSERT INTO broker_order_event_new SELECT id, intent_order_id, execution_report_id, provider, event_type, broker_order_id, status,
  detail_json, event_at, created_at FROM broker_order_event;
--> statement-breakpoint
