-- Shadow is an observation-only stage, distinct from paper performance and
-- real-money deployment approval.  Preserve the existing evaluation history
-- while allowing its own auditable enum value.
...
CREATE TABLE strategy_eval_run_new (
  ...
  eval_kind TEXT NOT NULL DEFAULT 'backtest'
    CHECK(eval_kind IN ('backtest','paper','shadow','live','walk_forward','holdout','recommendation')),
  period_start TEXT,
  period_end TEXT,
