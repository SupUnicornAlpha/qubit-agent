-- Reverse 0102: restore pre-running CHECK (data mapped to failed stays failed).
CREATE TABLE IF NOT EXISTS `mcp_call_log__down_0102` (
  ...
  `status` TEXT NOT NULL CHECK(
    `status` IN ('success', 'timeout', 'failed', 'sandbox_blocked')
  ),
  `error_code` TEXT,
  `latency_ms` INTEGER,
...
INSERT INTO `mcp_call_log__down_0102` (
  `id`, `workflow_run_id`, `agent_step_id`, `agent_definition_id`,
  `server_name`, `tool_name`, `trace_id`, `retry_count`, `transport`,
  `circuit_state`, `request_json`, `response_json`, `status`, `error_code`,
  `latency_ms`, `created_at`
)
SELECT
  `id`, `workflow_run_id`, `agent_step_id`, `agent_definition_id`,
  `server_name`, `tool_name`, `trace_id`, coalesce(`retry_count`, 0), `transport`,
  `circuit_state`, `request_json`, `response_json`,
  CASE
    WHEN `status` IN ('success', 'timeout', 'failed', 'sandbox_blocked') THEN `status`
    ELSE 'failed'
  END,
...
--> statement-breakpoint
ALTER TABLE `mcp_call_log__down_0102` RENAME TO `mcp_call_log`;
--> statement-breakpoint
