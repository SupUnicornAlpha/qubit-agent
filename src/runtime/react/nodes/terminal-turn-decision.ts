...
export function decideTerminalControl(input: {
  ...
  if (
    ...
    const retryCount = input.controlModeGapRetryCount ?? 0;
    const message = `你刚预告了下一步动作（拉数/并行工具/建计划后执行），但本轮没有发出合法 TOOL_CALL。请立刻用 <TOOL_CALL> 调用具体工具（如 market.snapshot.get / agent.invoke / research.thesis.write）；不要用纯文字假装已开始执行。${
