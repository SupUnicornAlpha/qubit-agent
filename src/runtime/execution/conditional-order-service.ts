...
export async function amendWaitingConditionalOrder(
  ...
  const intent = (
    await db.select().from(orderIntent).where(eq(orderIntent.id, input.orderIntentId)).limit(1)
  )[0];
  ...
  if (
    ...
    intent.orderType !== "stop_limit" &&
    intent.orderType !== "trailing_stop"
  ) {
    throw new Error("order_intent_is_not_conditional");
  }
  const task = (
    await db.select().from(executionTask).where(eq(executionTask.orderIntentId, intent.id)).limit(1)
  )[0];
  ...
  const patch = {
    ...
    ...(input.trailingOffsetPct !== undefined
      ? { trailingOffsetPct: input.trailingOffsetPct }
      : {}),
...
export function evaluateConditionalTrigger(
  input: ConditionalTriggerInput
): ConditionalTriggerResult {
  ...
  if (input.orderType === "trailing_stop") {
    ...
    const nextAnchorPrice =
      input.side === "sell"
        ? Math.max(priorAnchor, input.markPrice)
        : Math.min(priorAnchor, input.markPrice);
    const triggerPrice =
      input.side === "sell" ? nextAnchorPrice * (1 - offset) : nextAnchorPrice * (1 + offset);
    return {
      triggered:
        input.side === "sell" ? input.markPrice <= triggerPrice : input.markPrice >= triggerPrice,
      triggerPrice,
      nextAnchorPrice,
    };
  }
...
export async function processConditionalOrders(db: DbClient, nowIso: string): Promise<void> {
  const heldTasks = await db
    .select()
    .from(executionTask)
    .where(eq(executionTask.status, "held"))
    .limit(50);
  for (const task of heldTasks) {
    const child = (
      await db.select().from(orderIntent).where(eq(orderIntent.id, task.orderIntentId)).limit(1)
    )[0];
    ...
    const parent = (
      await db
        .select()
        .from(orderIntent)
        .where(eq(orderIntent.id, child.parentOrderIntentId))
        .limit(1)
    )[0];
    if (!parent) continue;
    if (parent.lifecycleStatus === "filled") {
      const conditional =
        child.orderType === "stop" ||
        child.orderType === "stop_limit" ||
        child.orderType === "trailing_stop";
      await db
        .update(orderIntent)
        .set({
      ...
      await appendConditionalEvent(
        db,
        task.id,
        "activate",
        { parentOrderIntentId: parent.id },
        nowIso
      );
      ...
      await db
        .update(executionTask)
        .set({ status: "cancelled", lastError: "parent_not_filled", updatedAt: nowIso })
        .where(eq(executionTask.id, task.id));
      await appendConditionalEvent(
        db,
        task.id,
  ...
  const waiting = await db
    .select()
    .from(executionTask)
    .where(eq(executionTask.status, "conditional_wait"))
    .limit(50);
  for (const task of waiting) {
    ...
    if (
      intent.orderType === "trailing_stop" &&
      result.nextAnchorPrice !== intent.trailingAnchorPrice
    ) {
      await db
        .update(orderIntent)
        .set({ trailingAnchorPrice: result.nextAnchorPrice, lifecycleUpdatedAt: nowIso })
    ...
    await db
      .update(executionTask)
      .set({ status: "pending", nextRetryAt: null, updatedAt: nowIso })
  ...
  for (const winner of filledOco) {
    ...
    const siblings = await db
      .select()
      .from(orderIntent)
      .where(eq(orderIntent.ocoGroupId, winner.ocoGroupId));
    for (const sibling of siblings) {
      ...
      const tasks = await db
        .select()
        .from(executionTask)
        .where(eq(executionTask.orderIntentId, sibling.id))
        .limit(1);
      const task = tasks[0];
      if (
        !task ||
        ...
        continue;
      if (
        (task.status === "waiting_ack" || task.status === "partially_filled") &&
        ...
        const submittedOrder = (
          await db
            .select()
            .from(brokerOrder)
            .where(eq(brokerOrder.orderIntentId, sibling.id))
            .limit(1)
        )[0];
      ...
      await db
        .update(executionTask)
        .set({ status: "cancelled", lastError: `oco_filled:${winner.id}`, updatedAt: nowIso })
        .where(eq(executionTask.id, task.id));
      await appendConditionalEvent(
        db,
        task.id,
        "cancel",
        { ocoGroupId: winner.ocoGroupId, winnerOrderIntentId: winner.id },
        nowIso
      );
      await appendAuditLog(db, {
