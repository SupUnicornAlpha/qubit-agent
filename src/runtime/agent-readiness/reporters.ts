...
export function renderMarkdownReport(snapshot: ReadinessSnapshot): string {
  ...
  for (const cat of ["A", "B", "C", "D", "LEGACY"] as const) {
    ...
    for (const id of view.metrics) {
      ...
      lines.push(`| ${id} | ${formatValue(id, value)} | ${icon} ${gradeText} | ${desc} |`);
    }
    lines.push("");
  }
...
function suggestForRed(id: string): string {
  const tips: Record<string, string> = {
    "A-1":
      "必备产物表为空——Agent 没真正落库；检查 reason → act → 落库链路是否被早退（respond_final 直接走捷径）。",
    "A-2": "产物字段未提及 goal 关键词；可能 Agent 根本没解析 goal，或在不同 ticker 上跑。",
    "A-3": "LLM judge 给低分；查看 'A-3 评分明细' issues，再回头改 prompt / 工具链。",
    "A-4":
      "产物之间引用断开（strategy 引用了不存在的 factor / order 引用了不存在的 strategy）；说明产物落库不一致。",
    "A-5":
      "效果质量门未过（如 IC/RankIC 太低、策略未完成回测、推荐未落快照）；优先补验证链路而不是只补文本总结。",
    "B-1": "必备工具未被调用；可能 prompt 没暴露工具描述、或 reason 节点没把工具列表给 LLM。",
    "B-2": "调用参数异常（qty<=0 / NaN / 非法日期）；加严工具入参 schema 校验。",
    "B-3":
      "工具失败率过高，看 tool_call_log error_message 聚类，常见是参数 schema 校验或资源未就绪。",
    ...
    "C-3-total":
      "token 消耗过大，考虑收敛 system prompt / 增加 cached_tokens 命中 / 减少不必要的工具枚举。",
    ...
    "D-4":
      "工作流进入终态但 orchestrator 没有生成 answerText；终态机不应把无回答任务标为 completed。",
    "D-5":
      "orchestrator 已有内部答案但没有投影到用户对话；检查 terminal response projector 与 workflow/chat 关联。",
  };
