...
`;
    const score = scoreSkillBody({
      description: "完整描述清晰的复杂流程，需要 5 步以上工具调用",
      bodyMd: good,
    });
  ...
  test("含 'this run' / commit SHA → 被扣分", () => {
    ...
    const clean = "# Skill\n## 步骤\n1. 拉数据\n2. 分析";
    const dirtyScore = scoreSkillBodyDetailed({
      description: "好的描述至少 60 字以保证检索能命中",
      bodyMd: dirty,
    });
    const cleanScore = scoreSkillBodyDetailed({
      description: "好的描述至少 60 字以保证检索能命中",
      bodyMd: clean,
    });
  ...
  test("description 含 '可能/也许' 扣分", () => {
    ...
    const sharp = scoreSkillBodyDetailed({
      description:
        "当 goal 是因子盘点且 universe ≥ 50 时使用，跑 5-step factor-discovery-promote-backtest 链",
      bodyMd: body,
    });
