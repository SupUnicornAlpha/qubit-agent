...
export function buildStrategyConsistencyReport(
  ...
  const latest = (kind: "backtest" | "paper" | "live") =>
    rows
  ...
  ).flatMap(([baselineKind, observedKind]) => {
      ...
      return [
        {
          ...
          actual,
          relativeDeviation,
          pass: relativeDeviation <= tolerance,
        },
      ];
    });
  });
  ...
  return {
    ...
    stages: Object.fromEntries(
      Object.entries(stages).map(([kind, row]) => [
        kind,
        row
          ? { id: row.id, pass: row.pass, qualityScore: row.qualityScore, createdAt: row.createdAt }
          : null,
      ])
    ),
    comparisons,
    ...
    pass:
      ...
      comparisons.every((item) => item.pass),
  };
}

export async function getStrategyConsistencyReport(
  input: {
    projectId: string;
