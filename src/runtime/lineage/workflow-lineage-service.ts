...
export async function buildWorkflowLineage(db: DbClient, workflowRunId: string) {
  const [
    ...
    db
      ...
      .where(eq(strategyComposition.workflowRunId, workflowRunId)),
    ...
    db
      ...
      .where(eq(recommendationSnapshot.workflowRunId, workflowRunId)),
  ...
  for (const factor of factors)
    add({
      id: nodeId("factor", factor.id),
      kind: "factor",
      label: factor.name,
      createdAt: factor.createdAt,
      asof: null,
      provider: factor.providerKey,
      freshness: null,
      metadata: { status: factor.status, horizon: factor.horizon, universe: factor.universe },
    });
  for (const version of versions)
    add({
      id: nodeId("strategy_version", version.id),
  ...
  for (const composition of compositions)
    add(
      {
        id: nodeId("strategy_composition", composition.id),
        kind: "strategy_composition",
        label: composition.name || composition.kind,
        createdAt: composition.createdAt,
        asof: null,
        provider: null,
        freshness: null,
        metadata: { kind: composition.kind, universe: composition.universe },
      },
      nodeId("strategy_version", composition.strategyVersionId),
      "composed_as"
    );
  for (const backtest of backtests)
    add(
      {
        id: nodeId("backtest", backtest.id),
        kind: "backtest",
        label: backtest.status,
        createdAt: backtest.startedAt,
        asof: backtest.endedAt,
        provider: backtest.providerId ?? backtest.engineKey,
        freshness: backtest.status,
        metadata: { datasetSnapshotId: backtest.datasetSnapshotId, engineKey: backtest.engineKey },
      },
      nodeId("strategy_version", backtest.strategyVersionId),
      "evaluated_by"
    );
  for (const evaluation of evaluations)
    add(
      {
        id: nodeId("evaluation", evaluation.id),
        kind: "evaluation",
        label: evaluation.evalKind,
        createdAt: evaluation.createdAt,
        asof: evaluation.periodEnd,
        provider: null,
      ...
      evaluation.backtestRunId ? nodeId("backtest", evaluation.backtestRunId) : rootId,
      "scored_by"
    );
  for (const recommendation of recommendations)
    add(
      {
        id: nodeId("recommendation", recommendation.id),
        kind: "recommendation",
        label: `${recommendation.symbol} ${recommendation.side}`,
        createdAt: recommendation.createdAt,
        asof: recommendation.dataAsof ?? recommendation.asof,
        provider: null,
        freshness: recommendation.status,
        metadata: {
  ...
  for (const order of orders)
    add(
      {
        id: nodeId("order_intent", order.id),
        kind: "order_intent",
        label: `${order.symbol ?? ""} ${order.side}`.trim(),
        createdAt: order.intentTime,
        asof: order.lifecycleUpdatedAt,
        provider: null,
        freshness: order.lifecycleStatus,
        metadata: { orderType: order.orderType, qty: order.qty, market: order.market },
      },
      nodeId("strategy_version", order.strategyVersionId),
      "executed_as"
    );
  for (const tool of tools)
    add(
      {
        id: nodeId("tool_call", tool.id),
        kind: "tool_call",
        label: tool.toolName,
        createdAt: tool.createdAt,
        asof: tool.createdAt,
        provider: tool.toolKind,
        freshness: tool.status,
        metadata: { agentDefinitionId: tool.agentDefinitionId, latencyMs: tool.latencyMs },
      },
      rootId,
      "used_tool"
    );
    ...
    add(
      {
        ...
        freshness: null,
        metadata: {
          agentDefinitionId: llm.agentDefinitionId,
