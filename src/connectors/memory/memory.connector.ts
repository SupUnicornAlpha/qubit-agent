 ...
 */
export abstract class BaseMemoryConnector extends BaseConnector implements MemoryConnector {
  abstract add(content: string, metadata: MemoryMetadata): Promise<MemoryRecord>;
  abstract search(query: string, filters: MemoryFilters, topK: number): Promise<MemoryRecord[]>;
  ...
  protected async onExecute<TOutput>(operation: string, payload: unknown): Promise<TOutput> {
    ...
    switch (operation) {
      case "add":
        return this.add(p.content as string, p.metadata as MemoryMetadata) as unknown as TOutput;
      case "search":
        return this.search(
          p.query as string,
